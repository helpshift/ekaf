%%
%% Pg2l creates and manages process groups on local node
%% for client library scenarioes.
%% It is a local progress group module, and fully compatiable with
%% (copied from) the distributed one pg2.
%%
...
create(Name) ->
    _ = ensure_started(),
    case ets:member(pg2l_table, {group, Name}) of
        false ->
...
init([]) ->
    pg2l_table = ets:new(pg2l_table, [ordered_set, protected, named_table]),
    {ok, #state{}}.
...
handle_call({join, Name, Pid}, _From, S) ->
    ets:member(pg2l_table, {group, Name}) andalso join_group(Name, Pid),
    {reply, ok, S};
handle_call({leave, Name, Pid}, _From, S) ->
    ets:member(pg2l_table, {group, Name}) andalso leave_group(Name, Pid),
    {reply, ok, S};
handle_call({delete, Name}, _From, S) ->
    delete_group(Name),
    {reply, ok, S};
handle_call(Request, From, S) ->
    error_logger:warning_msg("The pg2l server received an unexpected message:\n"
                             "handle_call(~p, ~p, _)\n", 
                             [Request, From]),
    {noreply, S}.
...
%%% One ETS table, pg2l_table, is used for bookkeeping. The type of the
...
assure_group(Name) ->
    Key = {group, Name},
    ets:member(pg2l_table, Key) orelse true =:= ets:insert(pg2l_table, {Key}).

delete_group(Name) ->
    _ = [leave_group(Name, Pid) || Pid <- group_members(Name)],
    true = ets:delete(pg2l_table, {group, Name}),
    ok.

member_died(Ref) ->
    [{{ref, Ref}, Pid}] = ets:lookup(pg2l_table, {ref, Ref}),
...
join_group(Name, Pid) ->
    Ref_Pid = {ref, Pid}, 
    try _ = ets:update_counter(pg2l_table, Ref_Pid, {4, +1}), true
    catch _:_ ->
            {RPid, Ref} = do_monitor(Pid),
            true = ets:insert(pg2l_table, {Ref_Pid, RPid, Ref, 1}),
            true = ets:insert(pg2l_table, {{ref, Ref}, Pid})
    end,
    Member_Name_Pid = {member, Name, Pid},
    try _ = ets:update_counter(pg2l_table, Member_Name_Pid, {2, +1})
    catch _:_ ->
            true = ets:insert(pg2l_table, {Member_Name_Pid, 1}),
            true = ets:insert(pg2l_table, {{pid, Pid, Name}})
    end.

leave_group(Name, Pid) ->
    Member_Name_Pid = {member, Name, Pid},
    try ets:update_counter(pg2l_table, Member_Name_Pid, {2, -1}) of
        N ->
            if 
                N =:= 0 ->
                    true = ets:delete(pg2l_table, {pid, Pid, Name}),
                    true = ets:delete(pg2l_table, Member_Name_Pid);
                true ->
                    ok
            end,
            Ref_Pid = {ref, Pid}, 
            case ets:update_counter(pg2l_table, Ref_Pid, {4, -1}) of
                0 ->
                    [{Ref_Pid,RPid,Ref,0}] = ets:lookup(pg2l_table, Ref_Pid),
                    true = ets:delete(pg2l_table, {ref, Ref}),
                    true = ets:delete(pg2l_table, Ref_Pid),
...
group_members(Name) ->
    [P || 
        [P, N] <- ets:match(pg2l_table, {{member, Name, '$1'},'$2'}),
        _ <- lists:seq(1, N)].
...
member_in_group(Pid, Name) ->
    case ets:lookup(pg2l_table, {member, Name, Pid}) of
        [] -> [];
        [{{member, Name, Pid}, N}] ->
...
member_groups(Pid) ->
    [Name || [Name] <- ets:match(pg2l_table, {{pid, Pid, '$1'}})].

all_groups() ->
    [N || [N] <- ets:match(pg2l_table, {{group,'$1'}})].

ensure_started() ->
    case whereis(?MODULE) of
        undefined ->
            C = {pg2l, {?MODULE, start_link, []}, permanent,
                 1000, worker, [?MODULE]},
...
simple_test() ->
    ...
    pg2l:create(T),
    pg2l:join(T, self()),
    ?assertEqual(pg2l:get_local_members(T), [self()]),
    ?assertEqual(pg2l:get_closest_pid(T), self()),
    pg2l:delete(T).
